...
class PartyTests(unittest.TestCase):
    ...
    def test_no_rsvp_yet(self):
        ...
        # details not in result.data
        result = self.client.get('/')
        print(result)
        self.assertNotIn(b'Party Details', result.data)
        # the users have not responded and should not be able to see party details
        # if they responded they will see party details

        # # print("FIXME")
        # result = self.client.get("/")
        # self.assertIn(b"Please RSVP", result.data)
        # because they have responded so they see the Please RSVP!
